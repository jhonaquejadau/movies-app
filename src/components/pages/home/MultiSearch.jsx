...
export const MultiSearch = () => {
    ...
    const options = types.map((type, index) => {
        return (
            <div key={index}
                ...
                className={`${selected === type ? 'bg-orange-500' : 'bg-[rgba(255,255,255,0.2)]'} px-6 text-[1rem] mr-2 capitalize`}
            >
                <p>{type}</p>
            </div>
        )
    ...
    return (
        <div className="w-[90%]">
            ...
            <div className={`${formData.length > 0 ? 'w-[100%] grid grid-cols-4 gap-2 max-[800px]:grid-cols-3 max-[600px]:grid-cols-2' : 'h-[100px] flex justify-center items-center bg-[rgba(255,255,255,0.2)] mb-4'} `}>
                {formData.length > 0 ?
                    movieSearch.filter(movie => movie.backdrop_path !== null).map(movie => {
                        return (
                            <div key={movie.id} className="relative w-[100%] h-[300px] hover:scale-[1.01]">
                                <img className="w-[100%] h-[100%] object-cover object-top" src={apiConfig.imageUrl('original',movie.poster_path)} alt={movie.title} />
                                <p className="absolute bottom-0 bg-[rgba(255,255,255,0.2)] w-[100%] p-2 font-bold">{movie.title}</p>
                            </div>
                        )
                    }) :
                    <p className="font-bold text-[1.75rem] opacity-50">Ooops there isn't any search!!</p>
                }
            </div>
        </div>
    )
