...
export const MoviesCarousel = ({ upcoming, apiConfig }) => {
  const recentMovies =
    upcoming && upcoming.filter((movie) => movie.release_date >= "2022-05-01");

  const data = recentMovies.map((movie) => {
    return (
      <div key={movie.id}>
        <img
          className="relative block m-auto w-[100%] h-[600px] object-cover object-top"
        ...
        <div className="flex flex-col items-start absolute bottom-0 p-12 text-slate-200 bg-gradient-to-t from-black to-[rgba(0,0,0,0)] w-[100%] h-[60%] max-[900px]:h-[45%] px-[6em]">
          <p className="text-5xl font-bold max-[1200px]:text-2xl">
          ...
          <div className="w-[50%] text-left max-[900px]:hidden">
            {movie.overview}
          </div>
          <Link to={`/movie-detail/${movie.id}`}>
            <button className="px-6 py-2 font-bold text-xl uppercase bg-[rgba(255,255,255,0.3)] rounded mt-2 hover:border-2 hover:border-white">
              more info
            </button>
          </Link>
        </div>
